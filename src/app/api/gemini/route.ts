  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        console.log("model in createChatResponseStream : ", model);
        ...
        if (fullBotResponse.trim() === "") {
          console.warn(
            `Gemini returned an empty response for chat ${chatId}. Not saving empty message pair.`
          );
        } else {
          try {
        ...
        console.error("Gemini streaming or processing error:", error);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { message, previous_conversations, model } = await req.json();
    ...
    try {
      ...
      const title = await handleNewChatCreation(
        isNewChatFlow,
        finalChatId,
        message,
        userId
      ...
      // Then start the stream after chat is created
